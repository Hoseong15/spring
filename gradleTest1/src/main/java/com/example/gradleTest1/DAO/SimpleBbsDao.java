...
public class SimpleBbsDao implements SimpleBbsDaoImple {
	...
	public SimpleBbsDto viewDao(String id) {
		...
		SimpleBbsDto dto = jdbcTemplate.queryForObject(sql, 
				new BeanPropertyRowMapper<SimpleBbsDto>(SimpleBbsDto.class) );
		return dto;
	}

	@Override
	public int writerDao(SimpleBbsDto simpleBbsDto) {
		String sql = "insert into simple_bbs(writer,title,content) values(?,?,?)";
		jdbcTemplate.update(sql, simpleBbsDto.getWriter(), simpleBbsDto.getTitle(), simpleBbsDto.getContent());
		return 0;
	}
